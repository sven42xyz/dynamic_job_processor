...
func setupRouter() *gin.Engine {
	...
	router := gin.New()
	logger = initTestLogger() // Verwenden Sie den Test-Logger
	return router
}
...
func TestCheckWritable(t *testing.T) {
	...
	defer tsOK.Close()
	config = viper.New()
	config.Set("target_system_url", tsOK.URL)
	...
	defer tsNotOK.Close()
	config.Set("target_system_url", tsNotOK.URL)
	...
	config.Set("target_system_url", tsNotFound.URL)
	...
	config.Set("target_system_url", "invalid-url")
	writable, err = checkWritable("test-uid")
	assert.Error(t, err)
	assert.False(t, writable)
}
...
func TestWriteData(t *testing.T) {
	...
	config = viper.New()
	config.Set("target_system_url", tsSuccess.URL)
	...
	defer tsError.Close()
	config.Set("target_system_url", tsError.URL)
	...
	config.Set("target_system_url", tsSuccess.URL) // Verwenden Sie eine gültige URL, um den HTTP-Aufruf zu ermöglichen
	...
	config.Set("target_system_url", "%invalid-url") // Ungültige URL
	err = writeData("test-uid", map[string]interface{}{"key": "value"})
	assert.Error(t, err)
...
func TestSaveAndRestorePendingJobs(t *testing.T) {
	...
	testJobs := []PendingJob{
		{Job: Job{UID: "job1", Data: map[string]interface{}{"a": 1}}, CreatedAt: time.Now()},
		{Job: Job{UID: "job2", Data: map[string]interface{}{"b": 2}}, CreatedAt: time.Now().Add(-time.Hour)},
	}
...
func TestProcessJobs(t *testing.T) {
	...
	jobsMutex.Lock()
	pendingJobs = append(pendingJobs, PendingJob{Job: Job{UID: "test-uid", Data: map[string]interface{}{"key": "value"}}, CreatedAt: time.Now()})
	jobsMutex.Unlock()
	...
	jobsMutex.Lock()
	pendingJobs = append(pendingJobs, PendingJob{Job: Job{UID: "another-uid", Data: map[string]interface{}{"key": "value"}}, CreatedAt: time.Now()})
	jobsMutex.Unlock()
